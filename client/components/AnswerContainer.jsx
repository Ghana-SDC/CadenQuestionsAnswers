...
export default class AnswerContainer extends React.Component {
  ...
  renderAnswers() {
    const array = this.props.answers.slice(0, this.state.viewCount);
    return array.map(answer => <ExpandedAnswer answer={answer} />);
  }
  ...
  preventNegatives() {
    if (this.props.answers.length - this.state.viewCount <= 0) {
      return;
    } else {
      return (
        <Button onClick={this.expandAnswers.bind(this)}>
          <I className="fas fa-angle-down" />
          See more answers ({this.props.answers.length - this.state.viewCount})
        </Button>
      );
    }
  }
